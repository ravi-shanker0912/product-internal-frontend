    ...
    Scaffold(
        ...
        Column(
            ...
            if (state.isLoading) {
                ...
                if (profile != null) {
                    SectionCard {
                        Text(profile.phoneE164, style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.SemiBold)
                        Row(
                            horizontalArrangement = Arrangement.spacedBy(8.dp),
                            modifier = Modifier.padding(top = 8.dp)
                        ) {
                            StatusBadge(text = profile.role, tone = com.example.driverappfrontend.ui.common.StatusTone.INFO)
                            StatusBadge(text = profile.status, tone = statusTone(profile.status))
                        }
                        if (profile.ratingCount > 0) {
                            Text(
                                text = "★ %.1f · %d ratings".format(profile.ratingAvg, profile.ratingCount),
                ...
                SectionCard {
                    Text("Edit details", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.SemiBold)
